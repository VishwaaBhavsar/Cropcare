import Link from "next/link";


function Navbar() {
    return (
        <>
        <div className="bg-white sticky flex flex-row items-center justify-between">
         <Link href='/'><img className=" ml-[7vw] mt-0 h-20 w-40" src="/logo.svg" /></Link>   
        <div className="flex flex-row gap-3 mr-20 mt-2.5">
            <h3 className="font-medium text-2xl mt-5">About us</h3>
        <button className="bg-[#117955] px-4 py-2 text-xl rounded-full text-white mt-1">Login</button>
        <button className="bg-[#117955] p-5 rounded-4xl text-white mt-1">Sign Up</button>
        </div>
        </div>
        </>
    );
}

export default Navbar;