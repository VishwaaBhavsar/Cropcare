...
const IndiaVegetableGrowingChart = () => {
  ...
  const fetchRegionalVegetables = async () => {
    ...
    try {
      ...
      // Create a prompt that asks for region-specific vegetables
      ...
      try {
        let parsedVegetables;
        
        // Extract JSON portion from the response if needed
        const jsonMatch = data.response.match(/\[\s*\{[\s\S]*\}\s*\]/);
        if (jsonMatch) {
          parsedVegetables = JSON.parse(jsonMatch[0]);
        } else {
          parsedVegetables = JSON.parse(data.response);
        }
        
        // Verify the expected format
        if (!Array.isArray(parsedVegetables) || parsedVegetables.length === 0) {
          throw new Error('Invalid data format');
        }
        
        // Set the vegetables data
        setVegetables(parsedVegetables);
      } catch (parseError) {
